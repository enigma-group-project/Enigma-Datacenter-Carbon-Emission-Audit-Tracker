# 🧪 Build · Validate · Troubleshoot · Test — Enigma-Datacenter-Carbon-Emission-Audit-Tracker

> **enigma-group-project** · student template (implement the TODOs)
> Repo: <https://github.com/enigma-group-project/Enigma-Datacenter-Carbon-Emission-Audit-Tracker> · GUI: <https://enigma-group-project.github.io/Enigma-Datacenter-Carbon-Emission-Audit-Tracker/> · Tracker: <https://enigma-group-project.github.io/Enigma-Datacenter-Carbon-Emission-Audit-Tracker/tracker/>
...
|---|-------|----------|-----------------|---------------|---------------|
| 1 | Registration | `contracts/IssuerRegistry.sol` | `forge test --match-contract IssuerRegistryTest` | `frontend/src/modules/issuer/` | `registerAuditor / deregisterAuditor / transferComplianceOfficer` |
| 2 | Emission report | `contracts/RecordRegistry.sol` | `forge test --match-contract RecordRegistryTest` | `frontend/src/modules/record/` | `recordEmissionReport` |
| 3 | Verification | `contracts/Verification.sol` | `forge test --match-contract VerificationTest` | `frontend/src/modules/verification/` | `verifyEmissionReport / verifyEmissionReportHash` |
| 4 | Transfer / revoke / audit | `contracts/AuditTrail.sol` | `forge test --match-contract AuditTrailTest` | `frontend/src/modules/audit/` | `transferFacility / revokeEmissionReport / amendReport / logAuditCheckpoint` |
...
### A1. Get the code
```bash
git clone https://github.com/enigma-group-project/Enigma-Datacenter-Carbon-Emission-Audit-Tracker.git
cd Enigma-Datacenter-Carbon-Emission-Audit-Tracker
...
```bash
gh run list  -R enigma-group-project/Enigma-Datacenter-Carbon-Emission-Audit-Tracker --workflow ci.yml --limit 1
gh run watch -R enigma-group-project/Enigma-Datacenter-Carbon-Emission-Audit-Tracker
```

### B2. Troubleshoot remotely
Open the failed run → **build-test** → expand the red step to read the log, or:
```bash
gh run view -R enigma-group-project/Enigma-Datacenter-Carbon-Emission-Audit-Tracker --log-failed
```
Same root causes as the table in **A4** (most often: forge-std install or a Solidity error).

### B3. Test the GUI on GitHub Pages
- 🌐 **https://enigma-group-project.github.io/Enigma-Datacenter-Carbon-Emission-Audit-Tracker/** — the live four-module web app (published by `.github/workflows/pages.yml` from `frontend/src`).
- 📊 **https://enigma-group-project.github.io/Enigma-Datacenter-Carbon-Emission-Audit-Tracker/tracker/** — the project task tracker.
...
- Run it on demand without a code change:
  ```bash
  gh workflow run "Auto-Grade" -R enigma-group-project/Enigma-Datacenter-Carbon-Emission-Audit-Tracker     # or the Actions ▸ Auto-Grade ▸ "Run workflow" button
  ```
