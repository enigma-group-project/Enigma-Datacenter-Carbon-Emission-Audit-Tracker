...
/// @title RecordRegistry — Slice 2 (EmissionReportRecorded  + state)  [STUDENT TEMPLATE]
...
contract RecordRegistry is IAttestationRegistry {
    ...
    /// @notice One-time link to the AuditTrail controller (complianceOfficer only).
    function setController(address controller_) external {
        // TODO(member2): require msg.sender == issuers.complianceOfficer() (else NotComplianceOfficer), require controller unset
        //               (else ControllerSet), then controller = controller_;
        revert("TODO(member2): implement setController");
    }

    /// @notice Create a record. onlyAuditor + nonReentrant. Reject duplicate id (AlreadyExists). Emit EmissionReportRecorded.
    function recordEmissionReport(bytes32 id, bytes32 dataHash, string calldata cid, address owner_, string calldata metadataURI)
        external onlyAuditor nonReentrant
    {
        // TODO(member2): if records[id].status != Status.None revert AlreadyExists();
        //               write the Attestation (status Active, issuedAt = block.timestamp); emit EmissionReportRecorded.
