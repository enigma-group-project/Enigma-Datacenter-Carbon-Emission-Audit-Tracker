...
contract VerificationTest is Test {
    ...
    function setUp() public {
        ...
        issuers.registerAuditor(issuer);
        vm.prank(issuer);
        records.recordEmissionReport(id, dataHash, "cid1", owner, "ipfs://meta");
    }

    function test_VerifyHashMatch() public view {
        assertTrue(verifier.verifyEmissionReportHash(id, dataHash));
    }

    function test_WrongHashFails() public view {
        assertFalse(verifier.verifyEmissionReportHash(id, keccak256("wrong")));
    }

    function test_FullVerifyActive() public view {
        (bool valid, IAttestationRegistry.Status status, address o, address i) = verifier.verifyEmissionReport(id, dataHash);
        assertTrue(valid);
        assertEq(uint256(status), uint256(IAttestationRegistry.Status.Active));
        assertEq(o, owner);
        assertEq(i, issuer);
    }
    // TODO(slice-3): assert verifyEmissionReport returns valid=false after a revoke.
}
